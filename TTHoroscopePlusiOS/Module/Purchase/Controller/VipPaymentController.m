...
#import "VipTitleView.h"
#import "TTBaseNavigationController.h"
#import "VipPaymentCell.h"

#import "XTPaymentPriceModel.h"
...
@property (nonatomic, strong) VipTitleView  *titleView;
...
@property (nonatomic, strong) NSArray <XTPaymentPriceModel *>*vipPriceArray;
...
- (void)reloadViewSelectorFromBg:(NSNotification *)notification {
    // 时间间隔判断
    NSNumber *time_intervel = [[NSUserDefaults standardUserDefaults] objectForKey:@"config_dict"][@"cloud"][@"refresh_time_interval"];
    if (!time_intervel) {
...
-(void)reloadViewSelector{
    [[XYManager sharedInstance] reloadConfigWithDelegate:self];
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[XYPaymentManager shareInstance] addPayTransactionObserver];
...
//    [SVProgressHUD show];
    @weakify(self);
    [[XYPaymentManager shareInstance] checkSubscriptionStatusComplete:^(BOOL isVip) {
...
- (void)dealloc {
    [[XYPaymentManager shareInstance] removePayTransactionObserver];
    [SVProgressHUD dismiss];
}
...
- (void)restoreButtonAction:(UIButton *)sender {
    [[XYPaymentManager shareInstance] refreshReceipt];
...
- (void)payButtonAction:(UIButton *)sender {
    [self payWithProduct:self.vipPriceArray.firstObject.payment_id];
    [self logIsPayButton:YES];
}

- (void)payWithProduct:(NSString *)productId {
    [SVProgressHUD show];
    [[XYPaymentManager shareInstance] paymentWithProductID:productId];
}
...
- (void)setupViews {
    ...
    self.titleView = [VipTitleView yg_loadViewFromNib];
    ...
    if (self.vipPriceArray && self.vipPriceArray.count > 1) {
        ...
        [self.subTabview registerNib:[UINib nibWithNibName:NSStringFromClass([VipPaymentCell class]) bundle:nil] forCellReuseIdentifier:NSStringFromClass([VipPaymentCell class])];
...
- (NSDictionary *)getConfigInfo {
    NSDictionary *dict = [[NSUserDefaults standardUserDefaults] objectForKey:@"config_dict"];
    NSDictionary *subscribe = dict[@"cloud"][@"subscribe"];
    
    NSString *limitVersion = subscribe[@"<=version"];

    NSString *current = [BaseTools publicBundleVersion];
    ...
    if (!subscribe || (compareResult == NSOrderedDescending) || (compareResult == NSOrderedSame)) {
        return   @{
                   ...
                   @"terms_of_services":@"http://xtoolapp.com/xtoolapp_terms.html",
                   @"privacy_policy_title":@"Privacy Policy",
                   @"privacy_policy":@"http://xtoolapp.com/xtoolapp_policy.html",
...
- (void)showWithState:(XYPaymentStatus)state isSuccess:(BOOL)success {
    ...
    if (success) {
        [XYPaymentManager shareInstance].isVip = YES;
...
- (void)requestDidSucceed:(BOOL)didSucceed info:(NSString *)info{
    ...
    NSDictionary *dict = [[NSUserDefaults standardUserDefaults] objectForKey:@"config_dict"];
    NSDictionary *subscribe = dict[@"cloud"][@"subscribe"];
    NSArray *payIDs = subscribe[@"subscribe_payment_id"];
    self.vipPriceArray = [NSArray yy_modelArrayWithClass:[XTPaymentPriceModel class] json:payIDs];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    @weakify(self);
    VipPaymentCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([VipPaymentCell class]) forIndexPath:indexPath];
    XTPaymentPriceModel *model = self.vipPriceArray[indexPath.row];
    model.index = indexPath.row + 1;
    cell.model = model;
    cell.payButtonAction = ^(XTPaymentPriceModel *model) {
        @strongify(self);
