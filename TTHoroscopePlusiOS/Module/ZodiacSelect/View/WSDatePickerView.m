...
- (void)setupUI {
    ...
    [self.datePicker mas_makeConstraints:^(MASConstraintMaker* make){
        ...
        make.bottom.equalTo(self.doneBtn.mas_top);
    }];
}
...
- (void)defaultConfig {
    ...
    for (NSInteger i=MINYEAR; i<=date_current.year; i++) {
...
- (NSArray *)getNumberOfRowsInComponent {
    ...
    NSDate* date_current = [NSDate date];
    NSInteger timeInterval = date_current.year - MINYEAR;
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    if (!customLabel) {
        ...
        [customLabel setFont:kFontTitle_L_17];//[UIFont systemFontOfSize:17]];
    }
    NSString *title;
    ...
    switch (self.datePickerStyle) {
        ...
        case DateStyleShowYearMonthDay:
            ...
            if (component==0) {
                title = [TTBaseTools monthNameWithIndex_en:[_monthArray[row] integerValue]];
            }
            if (component==1) {
                title = _dayArray[row];
            }
    ...
    if (!_datePickerColor) {
        _datePickerColor = [UIColor colorWithHex:0x333333];
    }
